"""Model and tokenizer loading for rfsn_v11 inference.

Ported from rfsn_v10/model_loader.py.
...
Differences vs rfsn_v10:
- Import errors raise :class:`rfsn_v11.errors.BackendError` instead of
  bare ``RuntimeError``.
- ``load_model_auto`` raises :class:`rfsn_v11.errors.ConfigurationError`
  for unknown ``backend`` values.
"""
...
def _require_mlx_lm() -> Any:
    """Import and return the ``mlx_lm`` module or raise :class:`BackendError`."""
    try:
        import mlx_lm
        return mlx_lm
...
def _require_transformers() -> Any:
    """Import and return the ``transformers`` module or raise :class:`BackendError`."""
    try:
        import transformers
        return transformers
    except ImportError as exc:
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        quant_config: Optional quantization config string
            ...
            attached to ``model._rfsn_quant_config`` for downstream use.
...
def load_model_auto(
    ...
    **kwargs: Any,
    ...
    Raises:
        :class:`~rfsn_v11.errors.ConfigurationError`: For unknown backend values.
        :class:`~rfsn_v11.errors.BackendError`: When the required backend
    ...
    raise ConfigurationError(
        f"Unknown backend {backend!r}.  Use 'mlx' or 'torch'."
    )
...
def get_model_info(model: Any) -> dict[str, Any]:
    ...
    Returns:
        Dict with keys ``architecture``, ``num_layers``, ``hidden_size``,
        ``num_attention_heads``, ``num_key_value_heads``, ``vocab_size``,
        ``quant_config``.  Values may be ``None`` if the backend does not
        expose them.
    """
    info: dict[str, Any] = {
        "architecture": None,
